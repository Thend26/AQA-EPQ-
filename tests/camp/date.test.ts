...
test("calculates camp days across month boundaries", () => {
  expect(campDayForDate("2026-08-01", "2026-07-16")).toBe(17);
});

test("calculates camp days across year boundaries", () => {
  expect(campDayForDate("2027-01-01", "2026-12-31")).toBe(2);
});

test("returns null before camp starts", () => {
  expect(campDayForDate("2026-07-15", "2026-07-16")).toBeNull();
});
...
test.each([
  ...
  expect(campDayForDate(date, startDate)).toBeNull();
});
